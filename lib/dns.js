    ...
    Dns.prototype.resolver = function (req, res) {
        ...
        $this.redis.get(key, function (err, value) {
            ...
            $this.nativeDNS($this.config.primary, question, hostname, req, res, function (err) {
                if (err) $this.logger.log('error', '%s:%s/%s - local - %s question:"%s" - %j -- ERROR:%j', req.connection.remoteAddress, req.connection.remotePort, req.connection.type, hostname, question, res.answer, err, $this.meta);

                $this.redis.set(key, JSON.stringify({ host: hostname, [question.type]: res.answer.map(e => e.data) }), (err) => {
                    if (err) $this.logger.log('error', 'dns/local SET hostname "%s" - err=%j', hostname, err, $this.meta);
                    else $this.logger.log('info', 'dns/local SET hostname "%s" - success', hostname, err, $this.meta);
                });
